...
def main(page: ft.Page):
    print("🚀 Iniciando App Chofer Web (v4.0 - Full Features)...")
        ...
        if id_op:
            ...
            if estado == "ENTREGADO":
                if not quien_recibio:
                    page.snack_bar = ft.SnackBar(ft.Text("⚠️ Por favor escribe quién recibe el paquete"), bgcolor="red")
                ...
                if not el_motivo:
                    page.snack_bar = ft.SnackBar(ft.Text("⚠️ Por favor escribe el motivo"), bgcolor="red")
                    page.snack_bar.open = True
                    page.update()
                    return
    ...
    def abrir_whatsapp(celular, guia, destinatario):
        if not celular or len(celular) < 6:
            page.snack_bar = ft.SnackBar(ft.Text("❌ No hay número de celular válido"))
            page.snack_bar.open = True
            page.update()
            return
        ...
        msg = urllib.parse.quote(f"Hola {destinatario}, soy del reparto de EK Logística. Tengo un envío para usted (Guía: {guia}). ¿Se encuentra en el domicilio?")
    ...
    def procesar_accion(id_op, nuevo_estado, detalle, n_guia):
        print(f"Guardando: {n_guia} -> {nuevo_estado}")
        conn = get_db_connection()
        if conn:
            try:
                ...
                page.snack_bar = ft.SnackBar(ft.Text(f"Guía {n_guia} actualizada"))
                page.snack_bar.open = True
                cargar_hoja_de_ruta()
            except Exception as e:
            ...
            try:
                ...
                if not filas:
                    ...
                    for f in filas:
                        ...
                        if es_cr:
                            info_pago = ft.Container(content=ft.Text(f"💰 ${monto}", color="white", weight="bold", size=12), bgcolor="red", padding=5, border_radius=5)
                        else:
                            info_pago = ft.Container(content=ft.Text("✅ OK", color="white", weight="bold", size=12), bgcolor="green", padding=5, border_radius=5)
                        ...
                        card = ft.Card(
                            content=ft.Container(
                                content=ft.Column([
                                    ...
                                    ft.Divider(),
                                    ft.Row([info_pago, ft.Text(f"📦 {bultos}", weight="bold")], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                                    ft.Container(height=5),
                                    ft.ElevatedButton(
    ...
    vista_principal = ft.Column([
        ft.Row([ft.Icon(name="local_shipping", color="blue", size=30), ft.Text("E.K. LOGISTICA", size=20, weight="bold")], alignment=ft.MainAxisAlignment.CENTER),
