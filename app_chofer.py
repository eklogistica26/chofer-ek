...
def main(page: ft.Page):
    ...
    # --- ELEMENTOS UI (TODO POSICIONAL, SIN "text=") ---
    # Usamos el texto directo, sin poner 'value=' ni 'text='
    ...
    def buscar_manual(e):
        ...
        if not term: return
        conn = get_db_connection(); col_resultado_busqueda.controls.clear()
        if conn:
            try:
                res = conn.execute(text("SELECT id, guia_remito, estado FROM operaciones WHERE guia_remito ILIKE :g"), {"g": f"%{term}%"}).fetchone()
                if res:
                    id_op, guia, est = res
                    col_resultado_busqueda.controls.append(ft.Container(content=ft.Column([ft.Text(f"Guía: {guia}", size=18, weight="bold"), ft.Text(f"Estado: {est}", color="blue"), ft.ElevatedButton(f"GESTIONAR {guia}", bgcolor="blue", color="white", on_click=lambda _: abrir_pantalla_edicion(id_op, guia))]), bgcolor="white", padding=10, border_radius=10))
                else: col_resultado_busqueda.controls.append(ft.Text("No encontrada", color="red"))
            except: pass
            finally: conn.close()
    ...
    tabs = ft.Tabs(selected_index=0, tabs=[ft.Tab(text="Mi Ruta", icon="list", content=tab_ruta), ft.Tab(text="Escáner", icon="qr_code", content=tab_buscar)], expand=1)
